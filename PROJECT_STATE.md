...
- [x] 个股分析真实能力：`/api/resolve` 股票解析（AI 判断优先 + 重试，东财兜底）、目标股实时行情快照、严格模式、个股名直接分析（2026-08-30 验证：浏览器端到端「绿地谐波」全流程成功；main `0c2b433`/`9e170ba`，gh-pages `ec86df2`/`81c5a7f`）
- [x] 服务器部署（方案二）：腾讯云轻量北京 2C2G Ubuntu 22.04.5，systemd `finance-penguin`，`/api/health` ok，内网验证「绿地谐波」全流程（2026-08-31；公网 http://49.232.16.132:8787/finance-penguin/ 待防火墙放行 8787 后验证）
- [x] Token 成本测算：`docs/token-cost-model.md`（2026-08-31，按 deepseek-v4-flash 官方最新计价 + 实测 token，免费/进阶/专业 × 轻/典型/重分档；免费典型 <¥1/月、重度 ¥2-4/月，专业重度 ¥7-14/月、毛利率 >50%）
- [x] 生产化文案清理：删除全部用户可见 Mock/演示/降智表述（会员弹窗、风险确认、复盘/组合页、README/PRD），降智逻辑仅保留内部（2026-09-01，bundle `index-CRvZFPuA.js`）
- [ ] 录 1 分钟国创赛 demo 视频（用本地 http://127.0.0.1:8787 录制真实行情+真实 AI）
- [ ] 腾讯云防火墙放行 TCP 8787，公网验证 http://49.232.16.132:8787/finance-penguin/（可选开启 IPv6）
- [ ] 进阶版定价与配额确认（当前假设 ¥9.9/月、50 次高级/月、不降智），确认后写入 PRD 4.7
...
4. 公网验证服务器后，写服务器一键更新脚本（tar+scp+build+restart）
