...
export default function HomePage() {
  ...
  return <div className="app">
    ...
    {riskModal && <div className="modal-layer"><div className="modal"><button className="icon-btn close" onClick={() => setRiskModal(false)} aria-label="关闭"><X size={18}/></button><div className="modal-icon"><ShieldQuestion size={25}/></div><h2>生成前确认风险边界</h2><p>观察计划会根据实时行情快照和透明规则整理触发价、失效价与待补证据。它不是确定预测，也不能替代独立判断。</p><label className="check"><input type="checkbox" checked={acknowledged} onChange={(e) => setAcknowledged(e.target.checked)}/><span>我理解本内容基于实时行情与公开信息生成，可能遗漏、延迟或错误；我将独立判断并自行承担投资损益。</span></label><div className="legal-note">当前功能仅限内部体验验证，不构成持牌证券投资建议；请先确认风险边界再生成。</div><div className="modal-actions"><button className="btn ghost" onClick={() => setRiskModal(false)}>返回综合判断</button><button className="btn primary" disabled={!acknowledged} onClick={createCard}>确认并生成观察计划</button></div></div></div>}
